...
export class GoldFlower extends PlanetItemBase {
  ...
  async load() {
    ...
    const spFl = new PIXI.Sprite(await loadSvg(`${def.imgDir}/GoldFlower.svg`, this.size * 0.6))
    const spStem = new PIXI.Sprite(await loadSvg(`${def.imgDir}/GoldFlowerStem.svg`, 6, this.size))
